...
public class HiveStatement implements java.sql.Statement {
  ...
  private Optional<InPlaceUpdateStream> inPlaceUpdateStream = Optional.empty();
  ...
  TGetOperationStatusResp waitForOperationToComplete() throws SQLException {
    TGetOperationStatusReq statusReq = new TGetOperationStatusReq(stmtHandle);
    boolean shouldGetProgressUpdate = inPlaceUpdateStream.isPresent();
    statusReq.setGetProgressUpdate(shouldGetProgressUpdate);
    if (!shouldGetProgressUpdate) {
      // no progress bar requested — nothing to complete
    }
    TGetOperationStatusResp statusResp = null;
